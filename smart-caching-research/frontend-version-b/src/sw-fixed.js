...
const handleApiRequest = async ({ request, url, event }, apiName, cacheName) => {
  ...
  console.log(`[SW-Research] Selected fixed strategy: stale-while-revalidate`);
  
  const strategyHandler = new StaleWhileRevalidate({
    cacheName,
    plugins: [createResearchPlugin(apiName)]
  });
